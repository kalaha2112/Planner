...
(() => {
  ...
  const BUILD_TAG = '2026-07-14 · stickers-9 (inspect2)';
  ...
  class Planner {
    ...
    // Drop stock memories / closet outfits whose stored image is essentially all
    // transparent (>98%) — the signature of the old knockout-to-black bug. New
    // cutouts never exceed 85% transparent, so this can't hit a good sticker.
    async sweepBlankStickers() {
      if (this._sweptBlanks) return;
      this._sweptBlanks = true;
      const dead = async (arr) => {
        const out = [];
        for (const it of arr || []) if (it && it.image && (await this._transparentFraction(it.image)) > 0.98) out.push(it.id);
        return out;
      };
      let changed = false;
      const badStock = await dead(this.data.stickerStock);
      if (badStock.length) {
        this.data.stickerStock = this.data.stickerStock.filter(s => !badStock.includes(s.id));
        this.data.placedStickers = (this.data.placedStickers || []).filter(p => !badStock.includes(p.stockId));
        changed = true;
      }
      for (const trip of Object.values(this.data.trips || {})) {
        const badClo = await dead(trip.closet);
        if (badClo.length) {
          trip.closet = (trip.closet || []).filter(o => !badClo.includes(o.id));
          (trip.stops || []).forEach(s => (s.itinerary || []).forEach(day => {
            if (day && Array.isArray(day.outfits)) day.outfits = day.outfits.filter(o => !badClo.includes(o.id));
          }));
          changed = true;
        }
      }
    ...
    autoCutout(dataUrl, opts) {
      ...
      return new Promise(resolve => {
        ...
        img.onload = () => {
          ...
          const n = px.length / 4;
          let srcAlpha = false;
          for (let i = 3; i < px.length; i += 4) { if (px[i] < 200) { srcAlpha = true; break; } }
          if (srcAlpha) { resolve(this._encodeAlpha(canvas, dataUrl)); return; }  // already transparent — keep it (PNG)
