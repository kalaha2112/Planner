...
(() => {
  ...
  const SYNC_BACKENDS = {
    ...
    e: {
      ...
      async create(body) {
        const res = await fetch(this.base, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body });
        if (!res.ok) throw _httpErr(this.name, res.status);
        const j = await res.json(); const id = j && (j.id || j.Id);
        if (!id) throw new Error('extendsclass returned no code.');
        return id;
      },
      async get(id) {
      ...
      async put(id, body) {
        const res = await fetch(this.base + '/' + encodeURIComponent(id), { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body });
        if (res.status === 404) throw _notFound();
        if (!res.ok) throw _httpErr(this.name, res.status);
      },
    },
    ...
    j: {
      ...
      async create(body) {
        const res = await fetch(this.base, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body });
        if (!res.ok) throw _httpErr(this.name, res.status);
        const id = res.headers.get('X-jsonblob') || ((res.headers.get('Location') || '').split('/').filter(Boolean).pop());
        if (!id) throw new Error('jsonblob exposed no code.');
        return id;
      },
      async get(id) {
        const res = await fetch(this.base + '/' + encodeURIComponent(id), { method: 'GET', headers: { 'Accept': 'application/json' }, cache: 'no-store' });
        if (res.status === 404) throw _notFound();
        if (!res.ok) throw _httpErr(this.name, res.status);
        return res.text();
      },
      async put(id, body) {
        const res = await fetch(this.base + '/' + encodeURIComponent(id), { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body });
        if (res.status === 404) throw _notFound();
        if (!res.ok) throw _httpErr(this.name, res.status);
      },
    },
  };
  const SYNC_ORDER = ['e', 'j'];   // create tries these in order
    ...
    async createSync() {
      ...
      for (const tag of SYNC_ORDER) {
        try {
          ...
          return;
        } catch (e) { lastErr = this.normErr(e); }
      }
      this._syncBusy = false;
      this.setSyncStatus('error', (lastErr && lastErr.message) || 'Could not create a code.'); this.bumpModal();
    }
