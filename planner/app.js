  ...
  class Planner {
    ...
    computeDates(trip) {
      ...
      trip.stops.forEach((stop, i) => {
        ...
        const lastDay = new Date(r.end);
        // an overnight train pushes the NEXT arrival a day on; the night itself
        // is the last thing you do here, so the day it leaves on is still yours
        const dayCount = Math.round((lastDay - r.start) / 86400000) + 1;
        // a day already planned on never falls off the end of the calendar
        const planned = (stop.itinerary || []).reduce(
          (n, day, di) => (day && (((day.items || []).length) || ((day.outfits || []).length))) ? di + 1 : n, 0);
        r.dayStart = new Date(r.start);
        r.dayCount = Math.max(1, planned, dayCount, nights + 1);
    ...
    buildCalendar(startDate, nights, itinerary, activeDay, closet, range) {
      ...
      while (cur <= end) {
        ...
        for (let dd = 1; dd <= daysIn; dd++) {
          ...
          // a whole one
          const half = (idx === 0 && range && range.arriveLeg) ? ' half half--in'
            : (idx === nights - 1 && range && range.departLeg) ? ' half half--out' : '';
    ...
    renderItineraryBody(trip, d, fmt) {
      ...
      const dayHalf = (i) => {
        if (!range) return '';
        if (i === 0 && range.arriveLeg && range.arriveLeg.arrText) return 'from ' + range.arriveLeg.arrText;
        if (i === daysN - 1 && range.departLeg && range.departLeg.depText) return 'until ' + range.departLeg.depText;
        return '';
      };
