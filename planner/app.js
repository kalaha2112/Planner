...
(() => {
  ...
  class Planner {
    ...
    _buildGlobe3DLand() {
      ...
      for (const f of feats) {
        ...
        for (const poly of polys) for (const ring of poly) {
          ...
          rings.push(ring.map(([lo, la]) => [la, lo]));
        }
      }
      if (g3.land) {
        g3.pivot.remove(g3.land);
        g3.mats = g3.mats.filter(m => m !== g3.land.material);
        g3.land.geometry.dispose(); g3.land.material.dispose();
      }
    ...
    _themeGlobe3D() {
      ...
      if (g3.land) setLine(g3.land, ink);
