...
(() => {
  ...
  const FLAT_W = 1.95, FLAT_H = 0.975;   // unrolled globe: plate carrée half-extents (2:1)
  ...
  class Planner {
    ...
    _initGlobe3D(wrap) {
      ...
      // Where each point lands once the globe is unrolled: a plate carrée, 2:1
      // like any wall map. Used as the morph target for the unfold transition.
      const flat = (lat, lng) => new T.Vector3((lng / 180) * FLAT_W, (lat / 90) * FLAT_H, 0);
      g3.flat = flat;
      ...
      // Lines are built from lat/lng rather than points so both the spherical
      // and the flat position can be baked in as attributes and mixed on the
      // GPU. A LineBasicMaterial can't morph, hence the small custom shader.
      const line = (latlngs, r, color, opacity) => {
        const sph = [], flt = [];
        for (const [la, lo] of latlngs) {
          const a = v(la, lo, r); sph.push(a.x, a.y, a.z);
          const b = flat(la, lo); flt.push(b.x, b.y, b.z);
        }
        const geo = new T.BufferGeometry();
        geo.setAttribute('position', new T.Float32BufferAttribute(sph, 3));
        geo.setAttribute('aFlat', new T.Float32BufferAttribute(flt, 3));
        const mat = new T.ShaderMaterial({
          transparent: true, depthTest: false, depthWrite: false,
          uniforms: { uFold: g3.uFold, uColor: { value: new T.Color(color) }, uOpacity: { value: opacity } },
          vertexShader: `
            attribute vec3 aFlat;
            uniform float uFold;
            void main() {
              vec3 p = mix(position, aFlat, uFold);
      ...
      for (let la = -60; la <= 60; la += 30) {
        ...
        grat.add(line(ll, R * 1.001, 0xffffff, .32));
      }
      ...
      // ---- country outlines from the shared 110m atlas ----
      ...
      //      _drawGlobe until the data lands.
      this._buildGlobe3DLand();

      this._themeGlobe3D();

      const resize = () => {
        const w = wrap.clientWidth || 1, h = wrap.clientHeight || w;
        renderer.setSize(w, h, false);
        camera.aspect = w / h; camera.updateProjectionMatrix();
      };
      g3.resize = resize;
      ...
      const tick = (now) => {
        ...
        if (g3.morphables && fold > 0) {
          for (const m of g3.morphables) m.position.lerpVectors(m.userData.sph, m.userData.flt, fold);
        ...
        if (g3.arcs && g3.arcs.length) {
          ...
          g3.arcs.forEach((line, i) => {
            ...
            line.geometry.setDrawRange(0, Math.round(local * line.userData.count));
          });
        }
        renderer.render(scene, camera);
      };
    ...
    _syncFold(cur) {
      ...
      const m = this.root.querySelector('.route.map-route') || this.root.querySelector('.main-map-wrap');
      ...
      // Anchor on a visible amount of it instead — a third of the viewport
      // filled, or the whole map where it is shorter than that — which lands
      // near a third of the park on both layouts rather than somewhere
      // different on each.
      //
      ...
      const need = Math.min(vh * 0.3, to.height || vh * 0.3);
      ...
      g3.uFold.value = p;
      canvas.style.left = (src.l + (to.left - src.l) * p) + 'px';
      canvas.style.top = (src.t + (to.top - src.t) * p) + 'px';
      canvas.style.width = Math.max(40, src.w + (to.width - src.w) * p) + 'px';
      canvas.style.height = Math.max(40, src.h + (to.height - src.h) * p) + 'px';
      // fade into the real map over the last stretch
      canvas.style.opacity = String(p > .88 ? Math.max(0, 1 - (p - .88) / .12) : 1);
      g3.resize();
    }

    // Build the country outlines once the shared 110m atlas is available.
    _buildGlobe3DLand() {
      const g3 = this._g3;
      if (!g3 || g3.land) return false;
      const topo = window.topojson, world = window.WORLD_ATLAS_DATA;
      if (!topo || !world) return false;
      const T = g3.T, R = g3.R;
      ...
      for (const f of feats) {
        ...
        for (const poly of polys) for (const ring of poly) {
          ...
          land.add(g3.line(ring.map(([lo, la]) => [la, lo]), R * 1.004, 0xffffff, .85));
        }
      }
      g3.pivot.add(land);
      g3.land = land;
    ...
    _themeGlobe3D() {
      ...
      if (g3.grat) g3.grat.children.forEach(l => setLine(l, ink));
      if (g3.land) g3.land.children.forEach(l => setLine(l, ink));
    ...
    _setGlobe3DStops(stops) {
      ...
      stops.forEach(([la, lo]) => {
        ...
        const fl = g3.flat(la, lo);
        const track = (m) => { m.userData.sph = pos.clone(); m.userData.flt = fl.clone(); g3.morphables.push(m); };
    ...
    _drawGlobe(svg, lat0deg, lng0deg, stops) {
      ...
      if (this._g3 && this._g3.built) {
        const g3 = this._g3;
        if (!g3.land) this._buildGlobe3DLand();     // atlas may have just arrived
