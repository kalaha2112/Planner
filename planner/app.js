  ...
  class Planner {
    ...
    _initGlobe3D(wrap) {
      ...
      for (let la = -60; la <= 60; la += 30) {
        const pts = [];
        for (let lo = -180; lo <= 180; lo += 4) pts.push(v(la, lo, R * 1.001));
        grat.add(line(pts, 0xffffff, .32));
      }
      for (let lo = -180; lo < 180; lo += 30) {
        const pts = [];
        for (let la = -90; la <= 90; la += 4) pts.push(v(la, lo, R * 1.001));
        grat.add(line(pts, 0xffffff, .32));
      }
      pivot.add(grat);
      g3.grat = grat;
    ...
    _buildGlobe3DLand() {
      ...
      for (const f of feats) {
        ...
        for (const poly of polys) for (const ring of poly) {
          ...
          land.add(g3.line(ring.map(([lo, la]) => g3.v(la, lo, R * 1.004)), 0xffffff, .85));
        }
      }
      g3.pivot.add(land);
      g3.land = land;
    ...
    _themeGlobe3D() {
      ...
      const ink = new T.Color(pick('--ink', '#1a1a1a'));
    ...
    _setGlobe3DStops(stops) {
      ...
      for (let i = 1; i < stops.length; i++) {
        const a = v(stops[i - 1][0], stops[i - 1][1], R);
        const b = v(stops[i][0], stops[i][1], R);
        const pts = [];
        const SEG = 48;
        for (let k = 0; k <= SEG; k++) {
          const t = k / SEG;
          const p = a.clone().lerp(b, t).normalize();
          p.multiplyScalar(R * (1 + 0.10 * Math.sin(Math.PI * t)));
          pts.push(p);
        }
        const geo = new T.BufferGeometry().setFromPoints(pts);
    ...
    _drawGlobe(svg, lat0deg, lng0deg, stops) {
      ...
      if (this._g3 && this._g3.built) {
        ...
        g3.pivot.rotation.y = (-lng0deg - 90) * Math.PI / 180;
        g3.pivot.rotation.x = lat0deg * Math.PI / 180;
