...
(() => {
  ...
  class Planner {
    ...
    initIntro() {
      ...
      this._introGlobeRefresh = () => { this._themeGlobe3D(); this._themeObjects3D(); this._buildIntroGlobe(svg); };   // stops added while intro open
    ...
    _initGlobe3D(wrap) {
      ...
      const tick = () => { g3.raf = requestAnimationFrame(tick); renderer.render(scene, camera); };
    ...
    _setGlobe3DStops(stops) {
      ...
      stops.forEach(([la, lo]) => {
        const m = new T.MeshBasicMaterial({ color: red });
        const d = new T.Mesh(dotGeo, m);
        d.position.copy(v(la, lo, R * 1.012));
        grp.add(d);
      });

      // route: great-circle-ish arcs lifted off the surface
      for (let i = 1; i < stops.length; i++) {
        ...
        grp.add(new T.Line(new T.BufferGeometry().setFromPoints(pts), mat));
      }
      g3.pivot.add(grp);
    }
    ...
    _drawGlobe(svg, lat0deg, lng0deg, stops) {
      ...
      if (this._g3 && this._g3.built) {
        ...
        if (key !== g3.stopsKey) { g3.stopsKey = key; this._setGlobe3DStops(stops); }
      }
    ...
    /* ============================================================
       ...
       Drives the overview blocks: each .ov-rise lifts in as it
       enters the viewport, the stat tiles cascade, the route strip
       slides its nodes in, and the meter bars fill from zero. The
       map gets a light parallax so the page has depth while
       scrolling. Fully additive: if GSAP failed to load, or the
       user prefers reduced motion, everything stays where the CSS
       already put it and the IntersectionObserver reveal above
       remains the only animation.
       ============================================================ */
    _initMotion() {
      const g = window.gsap, ST = window.ScrollTrigger;
      if (!g || !ST) return;
      if (this._webMag()) return;                 // ledger has its own entrances
      if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
      if (!this._motionReg) { g.registerPlugin(ST); this._motionReg = true; }

      // a re-render replaces the DOM these triggers pointed at
      ...
      this._motionCtx = g.context(() => {
        // immediateRender:false is load-bearing — a from() tween would otherwise
        // stamp opacity:0 the moment it's built, so anything whose trigger never
        // fires would be stranded invisible. With it off, nothing is touched
        // until the element actually scrolls in, and the CSS state is the
        // fallback. once:true then leaves it visible for good (no reverse, which
        // would blank the top blocks again the moment you scrolled back up).
        const st = (trigger, start) => ({ trigger, start, once: true, toggleActions: 'play none none none' });

        g.utils.toArray('.ov-rise').forEach((el) => {
          g.from(el, {
            opacity: 0, y: 26, duration: .62, ease: 'power3.out',
            immediateRender: false, scrollTrigger: st(el, 'top 92%')
          });
        });

        const tiles = g.utils.toArray('.ov-tiles .ov-tile');
        if (tiles.length) {
          g.from(tiles, {
            opacity: 0, y: 22, scale: .94, duration: .5, ease: 'back.out(1.6)', stagger: .07,
            immediateRender: false, scrollTrigger: st('.ov-tiles', 'top 92%')
          });
        }

        const nodes = g.utils.toArray('.ov-strip .ov-leg');
        if (nodes.length) {
          g.from(nodes, {
            opacity: 0, x: 34, duration: .5, ease: 'power3.out', stagger: .06,
            immediateRender: false, scrollTrigger: st('.ov-strip', 'top 94%')
          });
        }

        // bars grow from empty to whatever width the render already set
        g.utils.toArray('.ov-meter-fill').forEach((bar) => {
          g.from(bar, {
            width: 0, duration: .9, ease: 'power2.out',
            immediateRender: false, scrollTrigger: st(bar, 'top 96%')
          });
        });
        ...
        if (map) {
          g.to(map, {
            ...
            scrollTrigger: { trigger: map, start: 'top bottom', end: 'bottom top', scrub: .6 }
          });
        }
      }, scope);
    ...
    renderAppBook(trip, meta, travelers, d, fmt, nights, budget, milesNeeded) {
      ...
      <div class="app-ov">
        <div class="page" style="position:relative">
          ${this.renderOvBlocks(trip, meta, d, fmt, nights, budget, travelers)}
          ${this.renderMetaRange(trip)}
    ...
    /* ============================================================
       OVERVIEW BLOCKS — the designed top of the app overview: a hero
       masthead, a stat-tile grid, a scrollable route strip and two
       progress cards. Everything here is derived from real trip state
       (stops, legs, dates, packing, to-dos) — nothing is invented.
       Each block carries .ov-rise so the GSAP/ScrollTrigger layer in
       _initMotion() can reveal them on scroll.
       ============================================================ */
    renderOvBlocks(trip, meta, d, fmt, nights, budget, travelers) {
      const stops = trip.stops || [];
      const legs = [trip.outboundLeg, ...stops.map(s => s.leg)];

      // ---- countdown: whole days between today and departure ----
      const DAY = 86400000;
      const midnight = (x) => { const t = new Date(x); t.setHours(0, 0, 0, 0); return t; };
      let countLbl = '', countNum = '';
      if (trip.depart) {
        const diff = Math.round((midnight(trip.depart) - midnight(new Date())) / DAY);
        const end = trip.returnDate ? Math.round((midnight(trip.returnDate) - midnight(new Date())) / DAY) : diff;
        if (diff > 0) { countNum = String(diff); countLbl = diff === 1 ? 'day to go' : 'days to go'; }
        else if (end >= 0) { countNum = String(1 - diff); countLbl = 'day of the trip'; }
        else { countNum = '✓'; countLbl = 'trip complete'; }
      }

      const hero = `<header class="ov-hero ov-rise">
        <div class="ov-hero-eyebrow">${stops.length} stop${stops.length === 1 ? '' : 's'} · ${nights} night${nights === 1 ? '' : 's'} · ${travelers} traveller${travelers === 1 ? '' : 's'}</div>
        <h1 class="ov-hero-title">${esc(trip.label || 'Untitled trip')}</h1>
        ${countNum ? `<div class="ov-hero-count"><span class="n">${esc(countNum)}</span><span class="l">${esc(countLbl)}</span></div>` : ''}
      </header>`;

      // ---- stat tiles: facts the money summary below doesn't already show ----
      const legCount = legs.filter(Boolean).length;
      const tile = (v, c, mod) => `<div class="ov-tile${mod ? ' ' + mod : ''}"><div class="ov-tile-v">${esc(String(v))}</div><div class="ov-tile-c">${esc(c)}</div></div>`;
      const tiles = `<div class="ov-tiles ov-rise">
        ${tile(stops.length, stops.length === 1 ? 'stop' : 'stops')}
        ${tile(nights, 'nights')}
        ${tile(legCount, legCount === 1 ? 'leg' : 'legs', 'ov-tile--accent')}
        ${tile(travelers, travelers === 1 ? 'traveller' : 'travellers')}
      </div>`;

      // ---- route strip: origin → each stop → home, coloured by leg mode ----
      const node = (label, sub, mode, i) => {
        const col = mode ? MODE_HEX[mode] : '';
        return `<div class="ov-leg">
          ${i > 0 ? `<span class="ov-leg-line"${col ? ` style="--leg:${col}"` : ''}></span>` : ''}
          <div class="ov-node"${col ? ` style="--leg:${col}"` : ''}>
            <div class="ov-node-city">${esc(label)}</div>
            ${sub ? `<div class="ov-node-sub">${esc(sub)}</div>` : ''}
          </div>
        </div>`;
      };
      let strip = node(trip.originLabel || 'Home', d ? fmt(d.origin) : '', null, 0);
      stops.forEach((st, i) => {
        const r = d ? d.stops[i] : null;
        const n = Math.max(1, Number(st.nights) || 1);
        strip += node(st.city || 'Stop ' + (i + 1), r ? `${fmt(r.start)} · ${n}n` : `${n} night${n === 1 ? '' : 's'}`, (legs[i] || {}).mode, i + 1);
      });
      strip += node(trip.homeLabel || 'Home', d ? fmt(d.home) : '', (legs[stops.length] || {}).mode, stops.length + 1);
      const routeStrip = `<section class="ov-strip-wrap ov-rise">
        <div class="ov-block-hd">Route</div>
        <div class="ov-strip">${strip}</div>
      </section>`;

      // ---- progress: packing + pre-trip to-dos ----
      const packItems = Object.values(trip.packing || {}).flat();
      const packDone = packItems.filter(i => i && i.done).length;
      const todos = meta.todos || [];
      const todoDone = todos.filter(t => t && t.done).length;
      const pct = (a, b) => b ? Math.round(a / b * 100) : 0;
      const meter = (label, done, total, mod) => `<div class="ov-meter${mod ? ' ' + mod : ''}">
        <div class="ov-meter-hd"><span>${esc(label)}</span><span class="ov-meter-n">${done}/${total}</span></div>
        <div class="ov-meter-track"><span class="ov-meter-fill" style="width:${pct(done, total)}%"></span></div>
      </div>`;
      const meters = `<div class="ov-meters ov-rise">
        ${meter('Packed', packDone, packItems.length)}
        ${meter('Pre-trip to-do', todoDone, todos.length, 'ov-meter--alt')}
      </div>`;

      // the suitcase: a real 3D prop as the block's centrepiece, with the
      // packing figure beside it (falls back to just the figure without WebGL)
      const packPct = pct(packDone, packItems.length);
      const suitcase = `<section class="ov-object ov-rise">
        <div class="ov-object-stage" data-obj3d-kind="suitcase" data-obj3d-dist="4.0" data-obj3d-spin="0.28"></div>
        <div class="ov-object-copy">
          <div class="ov-block-hd">The suitcase</div>
          <div class="ov-object-figure">${packPct}<span>%</span></div>
          <p class="ov-object-note">${packDone} of ${packItems.length} item${packItems.length === 1 ? '' : 's'} packed</p>
          <button class="ov-object-btn" data-act="app-goto" data-i="0">Open packing</button>
        </div>
      </section>`;

      return hero + tiles + routeStrip + suitcase + meters;
    }
