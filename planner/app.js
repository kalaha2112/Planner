...
(() => {
  ...
  class Planner {
    constructor(root) {
      ...
      this.hPage = 0;               // itinerary(0) | transport&hotels(1) — the panel the horizontal slide is currently on
    ...
    renderLedger(trip, meta, travelers, d, fmt, nights, budget, milesNeeded) {
      ...
      <div class="ledger-stage">
        <div class="ledger-book${this.budgetOpen ? ' bill-open' : ''}" data-page="${page}">
          ${routeLeaf}
          <div class="leaf-group"><div class="leaf-sticky">${sharedHead}<div class="hstrip"><div class="htrack">${daysLeaf}${planLeaf}</div></div></div></div>
          ${packLeaf}
    ...
    magGoto(i) {
      ...
      if (i === 1 || i === 2) {
        ...
        const range = group.offsetHeight - window.innerHeight;   // the extra screen = the slide travel
        top = gTop + (i === 2 ? range : 0);
      } else {
    ...
    _onLedgerScroll() {
      ...
      const range = group.offsetHeight - vh;                     // one screen of slide travel
      let progress = range > 0 ? (-r.top) / range : 0;           // 0 itinerary … 1 transport
      progress = Math.max(0, Math.min(1, progress));
      track.style.transform = 'translateX(' + (-progress * 50) + '%)';   // 50% of a 200%-wide track = one panel
      this.hPage = progress >= 0.5 ? 1 : 0;
      // active section: Route while the stage is still low, Packing once it's high,
      // otherwise the stage half the slide is on
      let cur;
      if (r.top >= vh * 0.5) cur = 0;
      else if (r.bottom <= vh * 0.5) cur = 3;
      else cur = progress >= 0.5 ? 2 : 1;
      if (cur !== this.magIdx) {
        this.magIdx = cur;
    ...
    // Snap the two VERTICAL transitions so they land decisively, while the middle
    // horizontal slide (Itinerary↔Transport) stays free. The stage's scroll range:
    //   0 … itin(gTop) … [ horizontal slide ] … transport(gTop+range) … packing …
    // If the scroll rests inside Route↔Itinerary or Transport↔Packing, glide onto
    // the section in the direction you were scrolling — so a flick down from Home
    // lands on Itinerary, and past Transport lands on Packing.
    _snapVertical() {
      ...
      const itin = gTop, transport = gTop + range, packing = gTop + group.offsetHeight;
      const y = window.scrollY || 0;
      const dir = this._scrollDir || 0;
      let target = null;
      if (y > 2 && y < itin - 2) target = dir < 0 ? 0 : itin;                       // Route ↔ Itinerary
      else if (y > transport + 2 && y < packing - 2) target = dir < 0 ? transport : packing;   // Transport ↔ Packing
      if (target != null) this._snapScroll(target);
    }
    ...
    _snapScroll(target) {
      ...
      const dur = 240, t0 = performance.now(), ease = (t) => 1 - Math.pow(1 - t, 3);
      this._snapAnimating = true; this._snapAbort = false;
      const step = (now) => {
        if (this._snapAbort) { this._snapAnimating = false; return; }
        const t = Math.min(1, (now - t0) / dur);
        window.scrollTo(0, Math.round(start + dist * ease(t)));
        if (t < 1) requestAnimationFrame(step); else this._snapAnimating = false;
      };
      requestAnimationFrame(step);
    }
